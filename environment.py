...
class Environment:
    ...
    def step(self, action):
        ...
        # Log past state and current action
        ####### self.print_trace(action)
    ...
    def results(self, goal):
        ...
        if goal:
            if self.revisited == 0:
                self.print_path()
                self.no_revisits = True  # will not print for the 1st goal, but will start recording path
    ...
    def print_path(self):
        # Will only print the path if no cells are revisited
        if self.no_revisits:
            print(f'Shortest path length: {len(self.path)}, revisited: {self.revisited}.')
            # print(f'path:\n {self.path}')
            print('step, y, x')
            for i in range(len(self.path)):
                print(f'{i + 1}, {self.path[i][0]}, {self.path[i][1]}')

    # 2. trace in the environment, at each time unit, what was observed in surroundings, and what action was taken.
    def print_trace(self, action):
        print(f'\nSituation around y={self.y}, x={self.x}:\n{self.around[:, :, 0]}\n'
              f'Fires status:\n{self.around[:, :, 1]}\nAction selected: {action[0]}\n'
              f'status={self.t_last_state_vector[0, 0]:.5f}')
